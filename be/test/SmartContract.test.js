...
describe("SmartContract", () => {
  ...
  describe("Deployment", () => {
    ...
    it("Should deposit all rent cost", async () => {
      ...
      expect(await smartcontract.getbalanceOf(deployer)).to.equal(
        ethers.parseEther("4")
      );
    ...
    it("Should set the right deposit", async () => {
      ...
      expect(
        async () =>
          await smartcontract.depositContract(1, {
            value: ethers.parseEther("0.5"),
          })
      ).to.be.revertedWith("Deposit not enough");
      await smartcontract.depositContract(1, { value: ethers.parseEther("1") });
      expect(
        await smartcontract.getDepositContractByRenter(
          Number(123),
          deployer.address
        )
      ).to.equal(0);
    ...
    it("should close contract", async () => {
      ...
      await deployer.sendTransaction({
        to: smartcontract.target,
        value: ethers.parseEther("5"),
      });
      expect(await ethers.provider.getBalance(smartcontract.target)).to.equal(
        ethers.parseEther("5")
      );
  ...
  describe("Transfer", () => {
    it("Should add liquid", async () => {
      ...
      expect(await smartcontract.getbalanceOf()).to.equal(
        ethers.parseEther("5")
      );
  ...
  describe("Payment", () => {
    it("Should pay rent not enought", async () => {
      const { smartcontract } = await loadFixture(beforeTest);

      expect(async () => await smartcontract.payRentCost(1)).to.be.revertedWith(
        "Balancer not enought"
      );
    });
    ...
    it("Should has been withdrawn when get deposit", async () => {
      const { smartcontract } = await loadFixture(beforeTest);

      expect(
        async () => await smartcontract.payDepositToRenter(1)
      ).to.be.revertedWith("Deposit has been withdrawn");
    });

    it("Should pay deposit to rent", async () => {
      const { smartcontract } = await loadFixture(beforeTest);

      await smartcontract.depositContract(1, { value: ethers.parseEther("5") });

      await smartcontract.payDepositToRenter(1);
      const balanceOfContract = await smartcontract.getbalanceOf();

      expect(balanceOfContract).to.equal(ethers.parseEther("5"));
    });
  });
});
